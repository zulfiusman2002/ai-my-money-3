...
export function MiloBriefing({ name = 'there', body, facts = [], action, onAction, secondaryAction, onSecondary, busy = false }) {
  const greeting = new Date().getHours() < 12 ? 'morning' : new Date().getHours() < 18 ? 'afternoon' : 'evening';
  return <MiloCoach mode="core" eyebrow="Milo’s daily briefing" title={`Good ${greeting}, ${name} 👋`} body={body}
    facts={facts} action={busy ? 'Reading your numbers…' : action} onAction={onAction} secondaryAction={secondaryAction} onSecondary={onSecondary} motion={busy ? 'think' : 'wave'} tone="lavender" className="daily-briefing-v4" />;
}
