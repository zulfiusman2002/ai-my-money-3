    ...
    <div className="page page-wide learn-v3-page">
      <section className="academy-hero-v3 fade-up">
        <div className="academy-copy-v3">
          ...
          <p>Professor Milo turns your connected cash flow, goals and wealth patterns into the next useful lesson — not a generic course queue.</p>
          <div className="academy-stats-v3">
            ...
            <div className="academy-brain-stat-v2"><strong>{brain?.score ?? '—'}</strong><small>Money Pulse</small></div>
          </div>
        </div>
      ...
      {lesson && stage !== 'finished' && stage !== 'error' && <section className="academy-focus-v21 fade-up delay-1">
        <div className="academy-focus-status-v21"><span>{stage === 'ready' ? 'Your next lesson' : stage === 'done' ? 'Lesson completed' : 'Class in progress'}</span><strong>{lesson.title}</strong><small>{currentModuleTitle} · {stage === 'done' ? `+${result?.xp || 0} XP earned` : 'about 5 minutes'}</small></div>
        <div className="academy-focus-reason-v21"><span>Why Professor Milo chose it</span><p>{card?.reason || brain?.lesson?.reason || 'This is the most useful next idea in your learning path.'}</p></div>
        <div className="academy-focus-progress-v21"><div><span style={{width:`${stage === 'ready' ? Math.max(4, coursePct) : stage === 'done' ? 100 : Math.max(10, lessonProgress)}%`}}/></div><small>{stage === 'ready' ? `${coursePct}% of course complete` : stage === 'done' ? 'Ready for the next lesson' : `${Math.round(lessonProgress)}% of today’s lesson`}</small></div>
        {stage === 'ready' && <button className="btn btn-primary" onClick={()=>{setLessonStep(0);setStage('lesson');}}>Start class <Icon name="arrow" size={16}/></button>}
        {stage === 'done' && <button className="btn btn-primary" onClick={load}>Next lesson <Icon name="arrow" size={16}/></button>}
      </section>}
