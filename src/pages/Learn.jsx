...
export default function Learn() {
  ...
  const activeIndex = stage === 'quiz' || stage === 'done' ? totalLessonSteps : lessonStep + 1;
  ...
  return (
    <div className="page page-wide learn-v3-page">
      <section className="academy-hero-v3 fade-up">
        <div className="academy-copy-v3">
          ...
          <div className="academy-stats-v3">
            ...
            <div><strong>{progress.length}</strong><small>lessons done</small></div>
          </div>
        </div>
      ...
      {stage === 'finished' && <section className="academy-complete-v3"><MiloAvatar mode="learn" size={240}/><div><span className="academy-kicker">Path complete</span><h2>You’ve completed every available lesson.</h2><p>{card.reason}</p><button className="btn btn-primary" onClick={load}>Check for a new lesson</button></div></section>}

      {(stage === 'lesson' || stage === 'quiz') && lesson && <section className="lesson-experience-v3 fade-up">
        <aside className="lesson-guide-v3">
          <div className="guide-top-v3"><span>Today’s lesson</span><strong>{lesson.title}</strong><small>Module {lesson.module_id} · about 5 minutes</small></div>
          <div className="guide-professor-v3"><MiloAvatar mode="learn" size={230}/></div>
        ...
        <div className="module-map-v3">
          {modules.map((m,i)=>{
            ...
            return <article key={m.id} className={`module-node-v3 ${state}`}>
              ...
              <div><span>Module {i+1}</span><h3>{m.title}</h3><p>{m.description}</p></div>
              <div className="module-state-v3">{state === 'complete' ? 'Completed' : state === 'current' ? 'Up next' : 'Locked'}</div>
            </article>;
          })}
        </div>
      </section>
