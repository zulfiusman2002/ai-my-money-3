...
export default function NetWorth() {
  ...
  if (!trend.length || trend.at(-1).month !== currentTrendMonth || Math.abs(Number(trend.at(-1).net_worth) - comp.netWorth) > 1) {
    trend.push({ month: currentTrendMonth, net_worth: comp.netWorth, total_assets: comp.gross, total_liabilities: comp.totalLiabilities, is_live: true });
  }
  const mom = trend.length >= 2 ? Number(trend.at(-1).net_worth) - Number(trend.at(-2).net_worth) : null;

  return (
    <div className="page">
      <PageIntro eyebrow="Everything you own, minus everything you owe" title="Net Worth" subtitle="A single view across investments, property, gold, pensions, cash and liabilities." action={mom != null ? <div style={{ textAlign: 'right' }}><div className="t-label">vs last snapshot</div><div className="num-lg" style={{ color: mom >= 0 ? 'var(--c-green)' : 'var(--c-red)' }}>{mom >= 0 ? '+' : ''}{f(mom)}</div></div> : null} />
      ...
      <div className="card" style={{ marginBottom: 18 }}>
        ...
        {trend.length >= 2 ? (
            ...
            <AreaChart data={trend} margin={{ left: 8, right: 8 }}>
              <defs>
                <linearGradient id="nw" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#7657F5" stopOpacity={0.35} />
                  <stop offset="100%" stopColor="#7657F5" stopOpacity={0.02} />
                </linearGradient>
              </defs>
              ...
              <Area type="monotone" dataKey="net_worth" name="Net worth" stroke="#7657F5" strokeWidth={2.5} fill="url(#nw)" animationDuration={900} />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
          <div className="empty-state">
            ...
            <p className="empty-body">{trend.length === 1 ? 'One snapshot recorded. Check back next month to see the trend.' : "Run a Full Review on the Dashboard to capture this month's snapshot."}</p>
          </div>
        )}
      </div>
              ...
              <ResponsiveContainer width="100%" height={liabByType.length * 42}>
                <BarChart data={liabByType} layout="vertical" margin={{ left: 6 }}>
                  ...
                  <Bar dataKey="value" radius={[0, 6, 6, 0]} animationDuration={700}>
                    {liabByType.map((_, i) => <Cell key={i} fill="#D75555" opacity={1 - i * 0.15} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
