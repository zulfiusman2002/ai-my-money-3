...
import { defaultFinancialScope } from '../lib/financialSummary';
...
export default function NetWorth() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const firstError = [snaps, assets, liabs, months].find((r)=>r.error)?.error;
      ...
      const latestDate = Object.values(latest).map((x)=>x.snapshot_date).sort().at(-1) || null;
      setData({ latest: Object.values(latest), assets: assets.data || [], liabilities: scopedLiabilities, trend: months.data || [], latestDate });
    } catch(e) { setErr(e.message || 'Could not load net worth.'); setData({latest:[],assets:[],liabilities:[],trend:[],latestDate:null}); }
  };
  ...
  return (
    <div className="page page-wide">
      ...
      <DataStatus month={currentTrendMonth} updatedAt={data.latestDate}>Live net worth uses the latest value recorded for each asset and liability.</DataStatus>
      <MiloCoach mode="scientist" eyebrow="Scientist Milo · wealth lab"
        title={mom == null ? `Your net worth is ${f(comp.netWorth)}.` : `${mom >= 0 ? 'Net worth increased' : 'Net worth moved down'} by ${f(Math.abs(mom))} since the last snapshot.`}
        body="I separate market movement, asset valuations and liabilities so a chart change does not get mistaken for a change in your everyday cash flow."
        facts={[
          { label:'Assets', value:f(comp.gross), detail:'Everything currently tracked' },
          { label:'Liabilities', value:f(comp.totalLiabilities), detail:`${comp.gross ? Math.round(comp.totalLiabilities/comp.gross*100) : 0}% of assets` },
          { label:'Net position', value:f(comp.netWorth), detail:rangeDelta >= 0 ? `Up ${f(rangeDelta)} in this range` : `Down ${f(Math.abs(rangeDelta))} in this range` },
        ]}
        action="Review with Milo" onAction={runAI} motion={busy ? 'think' : 'point'} tone="blue" compact />
      ...
      <section className="card chart-card networth-trend-card">
        ...
        <div className="chart-legend-v41 detailed">
          <span><i style={{background:'#7657F5'}}/><b>Net worth</b><small>{f(comp.netWorth)} after liabilities</small></span>
          <span><i style={{background:'#47C9AA'}}/><b>Total assets</b><small>{f(comp.gross)} owned</small></span>
          <span><i className="dash" style={{background:'#D75555'}}/><b>Liabilities</b><small>{f(comp.totalLiabilities)} owed</small></span>
          <em>Hover or tap any month for exact values.</em>
        </div>
