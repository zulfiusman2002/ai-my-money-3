  ...
  return (
    <div className="auth-shell">
      <div className="auth-left">
        ...
        <h2 style={{ fontSize: 'clamp(1.6rem,4vw,2.2rem)', fontWeight: 700, color: 'var(--c-ink)', letterSpacing: '-.025em', lineHeight: 1.15, marginBottom: 20 }}>
          Start understanding<br />your money.
        </h2>
        <p style={{ color: 'var(--c-muted)', fontSize: '.9375rem', lineHeight: 1.65, marginBottom: 36 }}>
          Takes 3 minutes to set up. No card required.<br />Your data is private and protected by account-level access controls.
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {[['📊','Track income & expenses in one place'], ['📸','Update investments via screenshot'], ['🧠','AI briefing from your real numbers'], ['🎯','See if every goal is on track']].map(([icon, text]) => (
            <div key={text} style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <span style={{ color: 'var(--c-ink2)', fontSize: '.9rem' }}>{text}</span>
            </div>
          ))}
        </div>
      </div>
