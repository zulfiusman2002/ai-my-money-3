...
const CHAT_KEY = 'moneymilo_advisor_chat_v21';
const ASK_MODES = [
  ['decision','Make a decision','Affordability and trade-offs'],
  ['compare','Compare options','Side-by-side impact'],
  ['explain','Explain clearly','Teach me what this means'],
  ['plan','Build a plan','Turn a goal into steps'],
];
...
export default function Advisor(){
  ...
  const [askMode,setAskMode]=useState('decision');
  ...
  const send=async(e)=>{
    ...
    const apiMessages=next.map((message,index)=>({
      role:message.role,
      content: index===next.length-1 && message.role==='user'
        ? `MoneyMilo request mode: ${askMode}. ${answerToContext(message.content)}`
        : answerToContext(message.content),
    }));
    try{
    ...
    <div className="advisor-layout">
      ...
      <main className="advisor-main">
        ...
        <section className="chat-panel chat-panel-v1 chat-panel-v11">
          ...
          <div className="ask-mode-switch-v21" aria-label="Choose how Milo should answer">
            {ASK_MODES.map(([value,label,detail])=><button type="button" key={value} className={askMode===value?'active':''} onClick={()=>setAskMode(value)}><strong>{label}</strong><small>{detail}</small></button>)}
          </div>
          <form className="chat-form chat-form-v11 chat-form-v21" onSubmit={send}><textarea ref={textRef} rows="1" value={input} onKeyDown={keyDown} onChange={e=>setInput(e.target.value)} placeholder={askMode==='compare'?'What two options should Milo compare?':askMode==='explain'?'What should Milo explain in plain English?':askMode==='plan'?'What goal should Milo turn into a plan?':'Ask Milo about a purchase or money decision…'}/><button aria-label="Send question" disabled={chatBusy||!input.trim()}><Icon name="arrow" size={20}/></button></form>
          <div className="chat-compose-note-v11"><span>Enter to send · Shift + Enter for a new line</span><span>MoneyMilo 2.1 Premium Intelligence</span></div>
