...
import { AreaChart, Area, ResponsiveContainer, XAxis, YAxis, CartesianGrid, ReferenceDot, Tooltip } from 'recharts';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setData({ fin, goals: goals.data || [], comp, insights: insights.data || [], byCat, history: visibleHistory });
    })();
  }, [user.id]);
  ...
  if (!data) return <div className="page page-wide"><div className="skeleton" style={{ height: 230, marginBottom: 16 }} /><div className="grid g4">{[1,2,3,4].map((x)=><div key={x} className="skeleton" style={{height:130}} />)}</div></div>;
          ...
          <div className="chart-wrap">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.history} margin={{ top: 8, right: 4, left: 4, bottom: 0 }}>
                ...
                <CartesianGrid stroke="#E8E5F2" strokeDasharray="3 6" vertical={false}/>
                <XAxis dataKey="month" tickFormatter={shortMonth} minTickGap={24} tick={{fontSize:10,fill:'#8A889B'}} axisLine={false} tickLine={false}/>
                <YAxis hide domain={['dataMin - 5000','dataMax + 5000']}/>
                <Tooltip content={<MoneyChartTooltip formatter={f}/>}/>
                <Area type="monotone" dataKey="value" name="Net worth" stroke="#7657F5" strokeWidth={3} fill="url(#wealthGradient)" animationDuration={900}/>{data.history.length > 0 && <ReferenceDot x={data.history.at(-1).month} y={data.history.at(-1).value} r={5} fill="#7657F5" stroke="#fff" strokeWidth={3}/>} 
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </section>
