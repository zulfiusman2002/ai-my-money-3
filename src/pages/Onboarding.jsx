...
export default function Onboarding() {
  ...
  const screens = [
    <section className="onb-intro onb-v41-intro" key="intro"><div><div className="t-label">MoneyMilo setup</div><h1>Tell Milo about your money life — not a generic employment box.</h1><p>Add one person, a couple, a household, a business, or both. Milo adapts the questions and keeps personal and business money separate.</p><button className="btn btn-primary btn-lg" onClick={() => setStep(1)}>Start with Milo</button></div><MiloAvatar mode="core" size={360} motion="wave" glow/></section>,
  ...
  return <div className="onboarding-shell onboarding-v3 onboarding-v41">
    ...
    <section className="onboarding-main-v3">
      ...
      {step > 0 && step < TOTAL_STEPS - 1 && <footer className="onboarding-nav"><button className="btn btn-secondary" onClick={()=>{setErr('');setStep((s)=>Math.max(0,s-1));}}>Back</button><button className="btn btn-primary" onClick={goNext}>Continue</button></footer>}
    </section>
  </div>;
}
