...
export default function Landing(){
  return <div style={{minHeight:'100dvh',background:'radial-gradient(circle at 10% 0%,rgba(126,92,255,.16),transparent 28%),radial-gradient(circle at 92% 8%,rgba(79,211,190,.14),transparent 26%),#F8F8FD'}}>
    <nav style={{height:72,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 clamp(18px,5vw,64px)',position:'sticky',top:0,zIndex:20,background:'rgba(255,255,255,.78)',backdropFilter:'blur(20px)',borderBottom:'1px solid rgba(100,85,160,.1)'}}>
      <div className="brand"><span className="brand-mark">M</span><span className="brand-word">Money<span>Milo</span></span></div>
    ...
    <section style={{maxWidth:1180,margin:'0 auto',padding:'clamp(52px,8vw,96px) 24px 60px',display:'grid',gridTemplateColumns:'minmax(0,1.08fr) minmax(300px,.92fr)',alignItems:'center',gap:36}} className="landing-hero">
      <div>
    ...
    <section style={{maxWidth:1120,margin:'0 auto 80px',padding:'34px',borderRadius:30,background:'linear-gradient(135deg,#171724,#2D2740)',color:'white',display:'flex',alignItems:'center',justifyContent:'space-between',gap:20,flexWrap:'wrap'}}>
