...
export default function Goals() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (g.error) throw g.error; if (ast.error) throw ast.error;
      const sorted = [...(g.data || [])].sort((a,b)=>{
        if ((a.status==='achieved') !== (b.status==='achieved')) return a.status==='achieved' ? 1 : -1;
      ...
      setGoals(sorted);
      setBudget({ income: fin.totalIncome, expenses: fin.totalExpenses, month: fin.month, isFallback: fin.isFallback, completeness: fin.completeness });
      setAssets(ast.data || []);
    } catch(e) { setErr(e.message || 'Could not load goals.'); setGoals((x)=>x || []); }
  };
  useEffect(() => { load(); }, [user.id, scope]);
  ...
  const save = async () => {
    ...
    try {
      ...
      const response = form.id ? await supabase.from('goals').update(row).eq('id', form.id) : await supabase.from('goals').insert(row);
      if (response.error) throw response.error;
      setForm(null); await load();
      api.intelligence('snapshot').catch(() => {});
      window.dispatchEvent(new CustomEvent('moneymilo:data-changed'));
  ...
  return (
    <div className="page page-wide">
      ...
      <DataStatus month={budget.month || month} fallback={budget.isFallback}>{budget.isFallback ? `Goal affordability uses ${budget.month}, your latest complete budget month.` : `Goal affordability is linked to budget month ${budget.month || month}.`}</DataStatus>
      <MiloCoach mode="goals" eyebrow="Goals Milo · progress coach"
        title={headroom >= 0 ? `You still have ${f(headroom)} of monthly goal headroom.` : `Your goals need ${f(Math.abs(headroom))} more than this budget provides.`}
        body={brain?.goals?.behind ? `${brain.goals.behind} goal ${brain.goals.behind === 1 ? 'is' : 'are'} behind. MoneyMilo has linked this to your monthly cash flow and projector assumptions.` : headroom >= 0 ? 'Your goals are funded by a real monthly system, not just a target on a screen. Use the scenario controls to see what one small increase changes.' : 'We can protect the goals that matter most by adjusting contributions or improving cash flow.'}
        facts={[
          { label:'Net savings', value:f(netSavings), detail:`From ${budget.month || month}` },
          { label:'Committed', value:f(committed), detail:'Across active goals' },
          { label:'Headroom', value:f(headroom), detail:headroom >= 0 ? 'Still available each month' : 'Plan needs rebalancing' },
        ]}
        action="Create a goal" onAction={() => setForm({ goal_type:'Custom' })}
        secondaryAction="Check my goals" onSecondary={runAI} motion={busy ? 'think' : headroom >= 0 ? 'celebrate' : 'think'} tone="peach" compact />


      <section className="goal-connected-impact-v2">
        <div><span className="t-label">Connected impact</span><h2>Goals now feed the whole MoneyMilo system.</h2><p>Changing a contribution refreshes your goal headroom, Money Pulse, Projector starting plan, dashboard priorities and Professor Milo recommendation.</p></div>
        <div className="goal-connected-flow-v2"><span>Budget</span><Icon name="chevron" size={14}/><span>Goals</span><Icon name="chevron" size={14}/><span>Projector</span><Icon name="chevron" size={14}/><span>Learn</span></div>
        <button className="btn btn-secondary btn-sm" onClick={()=>refreshBrain()}>Refresh connection</button>
      </section>
      ...
      <div className="card" style={{ marginBottom: 18, display: 'flex', gap: 28, flexWrap: 'wrap', alignItems: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <MiloAvatar mode="goals" size={150} className="goal-strip-milo" motion="celebrate" />
      </div>
          ...
          {goals.map((g) => {
            ...
            return (
              <div key={g.id} className={`card goal-card ${goalVisual(g.goal_type)[1]}`} style={{ opacity: achieved ? 0.75 : 1 }}>
                ...
                <div className="goal-progress-summary-v42">
                  <div className="goal-progress-ring-v42" style={{'--goal-progress': `${t.pct * 3.6}deg`}}>
                    <div><strong>{t.pct.toFixed(0)}%</strong><span>complete</span></div>
                  </div>
                  <div className="goal-progress-copy-v42">
                    <span>Saved so far</span><strong>{f(t.current)}</strong><small>of {f(t.target)} target</small>
                    <div className="goal-progress-track-v42"><i style={{width:`${t.pct}%`}}/></div>
                  </div>
                </div>
      ...
      <div className="card milo-review-strip" style={{ marginTop: 18 }}>
        <MiloAvatar mode="goals" size={130} className="review-milo" />
