  ...
  return (
    <div className="page page-wide">
      ...
      <MiloCoach mode="future" eyebrow="Future Milo · scenario room"
        ...
        body={brain ? `Your connected plan currently has ${f(brain.monthly.committedGoals)} assigned to goals and ${f(brain.monthly.goalHeadroom)} remaining headroom. Change one assumption at a time to see the wider impact.` : 'This is a scenario, not a promise. Change one assumption at a time and I’ll show how saving, returns, property and inflation affect the destination.'}
        facts={[
      ...
      {brain && <section className="projector-connected-presets-v2">
        <div><span className="t-label">Connected monthly scenarios</span><h3>Start with a number already calculated elsewhere.</h3></div>
        <button className={monthly===Math.max(0,Math.round(brain.monthly.surplus))?'active':''} onClick={()=>setMonthly(Math.max(0,Math.round(brain.monthly.surplus)))}><span>Full surplus</span><strong>{f(Math.max(0,brain.monthly.surplus))}/mo</strong><small>All recorded income less expenses</small></button>
        <button className={monthly===Math.max(0,Math.round(brain.monthly.goalHeadroom))?'active':''} onClick={()=>setMonthly(Math.max(0,Math.round(brain.monthly.goalHeadroom)))}><span>After goals</span><strong>{f(Math.max(0,brain.monthly.goalHeadroom))}/mo</strong><small>Surplus after current goal commitments</small></button>
        <button onClick={()=>setMonthly(Math.max(0,Math.round(brain.monthly.surplus*.5)))}><span>Conservative</span><strong>{f(Math.max(0,brain.monthly.surplus*.5))}/mo</strong><small>Half of current monthly surplus</small></button>
      </section>}
