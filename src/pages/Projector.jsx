...
export default function Projector() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setSrc({ comp, investable, propertyWealth, netSavings: Math.max(0, income - expenses), debts0, debtApprox, budgetMonth: fin.month, budgetFallback: fin.isFallback });
  ...
  return (
    <div className="page">
      ...
      {src.budgetFallback && <div className="data-notice">Projection defaults use your latest complete budget month ({src.budgetMonth}).</div>}
      ...
      <div className="card" style={{ marginBottom: 18 }}>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={chart} margin={{ left: 8, right: 8 }}>
            ...
            {[3, 5, 10].map((y) => <ReferenceLine key={y} x={y} stroke="#D6D0E8" strokeDasharray="3 3" />)}
      ...
      <div className="card" style={{ marginBottom: 18 }}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={scenarios.base} margin={{ left: 8, right: 8 }}>
            ...
            <Tooltip formatter={(v) => f(v)} labelFormatter={(v) => `Year ${v}`} />
            <Legend wrapperStyle={{ fontSize: 11, fontFamily: 'Arial' }} />
            <Area type="monotone" dataKey="contributions" name="You put in" stackId="1" stroke="#65A8FF" fill="#65A8FF" fillOpacity={0.55} animationDuration={700} />
            <Area type="monotone" dataKey="growth" name="Compounding did" stackId="1" stroke="#7657F5" fill="#7657F5" fillOpacity={0.55} animationDuration={700} />
          </AreaChart>
        </ResponsiveContainer>
