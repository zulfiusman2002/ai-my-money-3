...
export default function Investments() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const failed = [snaps, ast, liab].find((row) => row.error);
      if (failed?.error) throw failed.error;
      setSnapshots(snaps.data || []);
      setAssets(ast.data || []);
      setLiabilities(liab.data || []);
    } catch (e) {
      setLoadError(e.message || 'Your investment picture could not be loaded.');
      setSnapshots([]);
    }
  };
  ...
  return (
    <div className="page page-wide">
      ...
      {loadError ? <div className="fatal-inline"><strong>Investment data could not be loaded</strong><p>{loadError}</p><button className="btn btn-secondary" onClick={load}>Try again</button></div> : <DataStatus updatedAt={snapshots?.[0]?.snapshot_date}>{snapshots?.length ? `${Object.keys(latestByType).length} portfolio type${Object.keys(latestByType).length === 1 ? '' : 's'} tracked · latest snapshot shown` : 'No broker snapshot yet — add one manually or from a screenshot'}</DataStatus>}
      ...
      <MiloCoach mode="investor" eyebrow="Investor Milo · portfolio desk"
        ...
        body={brain ? `This portfolio now contributes to your ${brain.score}/100 Money Pulse, net-worth model, projector and personalised learning path.${staleCount ? ` ${staleCount} update${staleCount > 1 ? 's are' : ' is'} stale.` : ''}` : staleCount ? `${staleCount} portfolio update${staleCount > 1 ? 's are' : ' is'} more than 45 days old. Fresh snapshots make Milo’s analysis more useful.` : 'Your latest broker data is ready to compare with property, cash, pensions and liabilities.'}
        facts={[
