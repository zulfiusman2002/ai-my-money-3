...
export default function Budget() {
  ...
  const sankey = useMemo(() => {
    ...
    push('Fixed', totals.byType.fixed, '#7657F5');
    push('Variable', totals.byType.variable, '#65A8FF');
    ...
    if (savings > 0) {
      ...
      for (const a of allocs) {
        ...
        nodes.push({ name: `${a.destination.replace(/_/g, ' ')} ${f(a.amount)}`, color: '#47C9AA' });
        links.push({ source: sIdx, target: nodes.length - 1, value: Number(a.amount) });
        allocated += Number(a.amount);
      }
      if (savings - allocated > 1) {
        nodes.push({ name: `unallocated ${f(savings - allocated)}`, color: '#D6D0E8' });
  ...
  if (!month || income === null) return <div className="page"><div className="skeleton" style={{ height: 300 }} /></div>;
        ...
        <div className="card" style={{ marginTop: 18 }}>
          ...
          <ResponsiveContainer width="100%" height={Math.max(260, sankey.nodes.length * 34)}>
            <Sankey data={sankey} node={<FlowNode />} nodePadding={28} nodeWidth={10}
              link={{ stroke: '#D6D0E8', strokeOpacity: 0.5 }}
