...
import { resolveFinancialMonth, defaultFinancialScope } from '../lib/financialSummary';
import { MiloCoach, PageIntro } from '../components/Milo';
import { DataStatus } from '../components/DataStatus';
import { useBrain } from '../context/BrainContext';
import {
  ResponsiveContainer, Tooltip, BarChart, Bar, XAxis, YAxis, Cell,
  RadialBarChart, RadialBar, PolarAngleAxis, PieChart, Pie, Legend, CartesianGrid, ComposedChart, Line,
} from 'recharts';

const HOUSEHOLD_CATEGORIES = ['housing', 'food', 'transport', 'family', 'lifestyle', 'health', 'debt', 'other'];
const BUSINESS_CATEGORIES = ['business_rent', 'payroll', 'contractors', 'software', 'inventory', 'equipment', 'marketing', 'professional_services', 'business_travel', 'business_insurance', 'tax', 'business_debt', 'business_other'];
...
export default function Budget() {
  ...
  const [scope, setScope] = useState(defaultFinancialScope(profile));
  const { brain, refresh: refreshBrain } = useBrain();
  ...
  const load = async () => {
    ...
    try {
      const [inc, exp, prev, al, hist] = await Promise.all([
        getMonthlyIncome(user.id, month, scope),
      ...
      const firstError = [exp, prev, al, hist].find((r) => r.error)?.error;
      if (firstError) throw firstError;
      const inScope = (row) => (row.scope || 'household') === scope;
      setIncome(inc.rows); setIncomeSource(inc.source); setExpenses((exp.data || []).filter(inScope));
      setPrevExpenses((prev.data || []).filter(inScope)); setAllocs(scope === 'household' ? (al.data || []) : []); setHistory(scope === 'household' ? (hist.data || []) : []);
    } catch (e) { setErr(e.message || 'Could not load this budget month.'); setIncome((x)=>x || []); }
  };
  useEffect(() => { resolveFinancialMonth(user.id, thisMonth(), scope).then((r) => setMonth(r.month)).catch((e)=>{setErr(e.message);setMonth(thisMonth());}); }, [user.id, scope]);
  useEffect(() => { if (month) load(); }, [user.id, month, scope]);
  ...
  const budgetTrend = useMemo(() => {
    const rows = history.map((x) => ({
      month: x.month, fixed: Number(x.fixed_expenses || 0), variable: Number(x.variable_expenses || 0), oneTime: Number(x.one_time_expenses || 0), total: Number(x.total_expenses || 0), savings: Number(x.total_savings || 0),
    }));
    const current = { month, fixed: totals.byType.fixed, variable: totals.byType.variable, oneTime: totals.byType['one-time'], total: totals.te, savings: Math.max(0, totals.net) };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (table === 'income') {
        const data = { user_id: user.id, month, name: row.name?.trim() || 'Income source', amount: Number(row.amount), type: row.type || 'salary', scope };
        let response;
        if (row.id && incomeSource === 'standard') {
          const records = await materialiseIncome(user.id, month, scope);
          const materialised = records.find((r) => r.source_id === row.id);
          response = materialised ? await supabase.from('income_records').update(data).eq('id', materialised.id) : await supabase.from('income_records').insert(data);
        } else if (row.id) response = await supabase.from('income_records').update(data).eq('id', row.id);
        else response = await supabase.from('income_records').insert(data);
        if (response.error) throw response.error;
      } else if (table === 'expense') {
        const data = { user_id: user.id, month, description: row.description, category: row.category, amount: Number(row.amount), type: row.type, recurring: row.type === 'fixed', scope };
        const response = row.id ? await supabase.from('expenses').update(data).eq('id', row.id) : await supabase.from('expenses').insert(data);
        if (response.error) throw response.error;
      } else {
        const data = { user_id: user.id, month, destination: row.destination, amount: Number(row.amount) };
        const response = row.id ? await supabase.from('savings_allocations').update(data).eq('id', row.id) : await supabase.from('savings_allocations').insert(data);
        if (response.error) throw response.error;
      }
      setEditing(null); await load();
      api.intelligence('snapshot').catch(() => {});
      window.dispatchEvent(new CustomEvent('moneymilo:data-changed')); refreshBrain({ silent: true });
  ...
  const remove = async (table, id) => {
    ...
    if (table === 'income' && incomeSource === 'standard') {
      ...
      const rows = await materialiseIncome(user.id, month, scope);
  ...
  const copyPrevious = async () => {
    ...
    const existingKeys = new Set(expenses.map((e)=>`${String(e.description).toLowerCase()}|${e.category}|${Number(e.amount)}`));
    const rows = fixed.filter((e)=>!existingKeys.has(`${String(e.description).toLowerCase()}|${e.category}|${Number(e.amount)}`)).map((e) => ({
      user_id: user.id, month, description: e.description, category: e.category,
      amount: e.amount, type: e.type, recurring: e.recurring, scope,
    }));
    if (!rows.length) { setErr('Those recurring expenses are already in this month.'); return; }
    ...
    return (
      <div className="card fade-up" style={{ marginBottom: 18 }}>
        ...
        <div className="grid g3" style={{ marginTop: 12 }}>
          {table === 'income' && (<>
            <div className="field"><label>Source</label><input value={row.name || ''} readOnly={Boolean(row.id)} onChange={(e) => set('name', e.target.value)} className={row.id?'field-static-v1':''}/>{row.id&&<small className="field-hint-v1">Income source names are managed in onboarding/settings; this edit changes the monthly amount only.</small>}</div>
          ...
          {table === 'expense' && (<>
            ...
            <div className="field"><label>Category</label><select value={row.category || 'other'} onChange={(e) => set('category', e.target.value)}>
              {(scope === 'business' ? BUSINESS_CATEGORIES : HOUSEHOLD_CATEGORIES).map((c) => <option key={c}>{c.replace(/_/g, ' ')}</option>)}</select></div>
  ...
  return (
    <div className="page page-wide">
      ...
      <PageIntro eyebrow={scope === 'business' ? 'Business revenue and operating costs' : 'Where the month went — and where it should go'} title={scope === 'business' ? 'Business Budget' : 'Budget'} subtitle={scope === 'business' ? 'A separate monthly view of business revenue, operating costs and operating remainder.' : 'A clear monthly view of household income, spending and the money you deliberately put to work.'} action={<div className="budget-head-actions-v41">{hasBusiness && <div className="scope-switch-v41"><button className={scope==='household'?'active':''} onClick={()=>setScope('household')}>Household</button><button className={scope==='business'?'active':''} onClick={()=>setScope('business')}>Business</button></div>}<button className="chip" onClick={() => setMonth(shiftMonth(month, -1))}>←</button><span className="budget-month-v41">{monthName(month)}</span><button className="chip" onClick={() => setMonth(shiftMonth(month, 1))} disabled={month >= thisMonth()}>→</button><button className="btn btn-primary btn-sm" onClick={() => setEditing({ table:'expense', row:{ category: scope==='business'?'business_other':'other', type:'variable', description:'', amount:'', scope } })}>+ Add expense</button></div>} />
      <DataStatus month={month}>{scope === 'business' ? `Editing business records for ${monthName(month)}` : `Editing household records for ${monthName(month)}`}</DataStatus>
      <MiloCoach mode="builder" eyebrow="Builder Milo · monthly foundations"
        title={totals.net >= 0 ? `${scope === 'business' ? 'The operating remainder is' : 'You have'} ${f(totals.net)}${scope === 'business' ? '.' : ' left to put to work.'}` : `This ${scope === 'business' ? 'business month' : 'month'} is ${f(Math.abs(totals.net))} over budget.`}
        body={brain && scope === 'household' ? `This budget now drives a ${brain.score}/100 Money Pulse, ${brain.goals.behind} goals behind and Professor Milo’s next lesson.` : totals.net >= 0 ? 'I’ve separated income, spending and allocations so you can see exactly what is available before committing it.' : 'Let’s repair the plan by checking the largest categories first — without judging the spending that matters to you.'}
        facts={[
          { label:scope === 'business' ? 'Revenue' : 'Income', value:f(totals.ti), detail:`Recorded for ${monthName(month)}` },
          { label:'Built costs', value:f(totals.te), detail:`${totals.ti ? Math.round(totals.te/totals.ti*100) : 0}% of income` },
          { label:scope === 'business' ? 'Operating remainder' : 'Available', value:f(Math.max(0,totals.net)), detail:scope === 'business' ? `${totals.rate.toFixed(0)}% operating margin` : `${totals.rate.toFixed(0)}% savings rate` },
        ]}
        action="Add an expense" onAction={() => setEditing({ table:'expense', row:{ category:'other', type:'variable', description:'', amount:'' } })}
        secondaryAction="Ask Milo to review" onSecondary={runInsights} motion={busy ? 'think' : 'point'} tone="gold" compact />

      {month !== thisMonth() && <div className="month-edit-notice"><span>You are viewing and editing <strong>{monthName(month)}</strong>. Any income, expense or allocation changes will be saved to this historical month only.</span><button onClick={() => setMonth(thisMonth())}>Go to {monthName(thisMonth())}</button></div>}
      ...
      {incomeSource === 'standard' && income.length > 0 && (
        <div className="card fade-up" style={{ marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, borderColor: 'var(--c-gold)' }}>
          ...
          <button className="btn btn-primary" style={{ padding: '8px 20px', fontSize: '.72rem' }} disabled={busy}
            onClick={async () => {
              setBusy(true);
              try { await materialiseIncome(user.id, month, scope); await load(); }
              finally { setBusy(false); }
            }}>
      ...
      <div className="grid g4">
        {[[scope === 'business' ? 'Revenue' : 'Income', f(totals.ti)], [scope === 'business' ? 'Operating costs' : 'Expenses', f(totals.te)], [scope === 'business' ? 'Operating remainder' : 'Net savings', f(totals.net)]].map(([l, v]) => (
        ...
        <div className="card" style={{ position: 'relative' }}>
          <div className="t-label">{scope === 'business' ? 'Operating margin' : 'Savings rate'}</div>
      ...
      <div className="budget-visual-grid">
        <section className="card chart-card">
          ...
          <div className="medium-chart"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={budgetTrend} margin={{top:18,right:18,left:2,bottom:4}}><defs><linearGradient id="saveLine" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#47C9AA"/><stop offset="100%" stopColor="#7657F5"/></linearGradient></defs><CartesianGrid stroke="#E8E5F2" strokeDasharray="3 6" vertical={false}/><XAxis dataKey="month" tickFormatter={(v)=>new Date(v+'-02').toLocaleDateString('en-GB',{month:'short'})} tick={{fontSize:11,fill:'#8A889B'}} axisLine={false} tickLine={false}/><YAxis tickFormatter={(v)=>`${sym}${Math.round(v/1000)}k`} tick={{fontSize:10,fill:'#8A889B'}} axisLine={false} tickLine={false} width={50}/><Tooltip formatter={(v,n)=>[f(v),n]} labelFormatter={(v)=>monthName(v)}/><Bar dataKey="fixed" name="Fixed costs" stackId="spend" fill="#7657F5" radius={[7,7,0,0]}/><Bar dataKey="variable" name="Variable" stackId="spend" fill="#65A8FF"/><Bar dataKey="oneTime" name="One-time" stackId="spend" fill="#FF9F77"/><Line type="monotone" dataKey="savings" name="Net savings" stroke="url(#saveLine)" strokeWidth={4} dot={{r:4,fill:'#47C9AA',stroke:'#fff',strokeWidth:2}} activeDot={{r:7}}/></ComposedChart></ResponsiveContainer></div><div className="budget-legend-v1"><span><i style={{background:'#7657F5'}}/>Fixed costs</span><span><i style={{background:'#65A8FF'}}/>Variable</span><span><i style={{background:'#FF9F77'}}/>One-time</span><span><i style={{background:'#47C9AA'}}/>Net savings</span></div>
        </section>
        <section className="card chart-card">
          <div className="chart-card-head compact"><div><div className="t-label">{scope === 'business' ? 'Operating cost mix' : 'Where savings go'}</div><h3>{scope === 'business' ? `${f(totals.te)} total costs` : `${f(Math.max(0,totals.net))} available this month`}</h3></div></div>
          {scope === 'household' ? (allocationData.length ? <div className="allocation-chart-wrap"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={allocationData} dataKey="value" nameKey="name" innerRadius="58%" outerRadius="82%" paddingAngle={3}>{allocationData.map((_,i)=><Cell key={i} fill={['#7657F5','#47C9AA','#65A8FF','#FF9F77','#D9A321','#A76BEF'][i%6]}/>)}</Pie><Tooltip formatter={(v)=>f(v)}/><Legend iconType="circle" wrapperStyle={{fontSize:12,textTransform:'capitalize'}}/></PieChart></ResponsiveContainer><div className="donut-center"><strong>{Math.round(Math.max(0,totals.net) ? Math.min(100,totals.ta/Math.max(1,totals.net)*100) : 0)}%</strong><span>allocated</span></div></div> : <div className="empty-state"><div className="empty-title">Give your savings a job</div><p className="empty-body">Allocate savings to goals, investments or your emergency fund.</p></div>) : (catData.length ? <div className="allocation-chart-wrap"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={catData} dataKey="value" nameKey="name" innerRadius="58%" outerRadius="82%" paddingAngle={3}>{catData.map((_,i)=><Cell key={i} fill={['#7657F5','#47C9AA','#65A8FF','#FF9F77','#D9A321','#A76BEF'][i%6]}/>)}</Pie><Tooltip formatter={(v)=>f(v)}/><Legend iconType="circle" wrapperStyle={{fontSize:12,textTransform:'capitalize'}}/></PieChart></ResponsiveContainer><div className="donut-center"><strong>{f(totals.te)}</strong><span>operating costs</span></div></div> : <div className="empty-state"><div className="empty-title">Add normal business costs</div><p className="empty-body">Payroll, software, rent and other operating costs appear here.</p></div>)}
        </section>
      </div>
      ...
      <div className="grid g2" style={{ marginTop: 18 }}>
        <div className="card">
          ...
          {catData.length ? <div className="allocation-chart-wrap budget-category-donut"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={catData} dataKey="value" nameKey="name" innerRadius="57%" outerRadius="82%" paddingAngle={3}>{catData.map((_,i)=><Cell key={i} fill={['#7657F5','#47C9AA','#65A8FF','#FF9F77','#D9A321','#A76BEF','#54B8D8','#7B89A8'][i%8]}/>)}</Pie><Tooltip formatter={(v)=>f(v)}/><Legend iconType="circle" layout="vertical" verticalAlign="middle" align="right" wrapperStyle={{fontSize:11,textTransform:'capitalize',lineHeight:'24px'}}/></PieChart></ResponsiveContainer><div className="donut-center"><strong>{f(totals.te)}</strong><span>spent</span></div></div> : <div className="empty-state"><div className="display">Nothing yet</div><p>Add this month's expenses below.</p></div>}
        </div>
        <div className="card">
