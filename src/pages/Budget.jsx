...
export default function Budget() {
  ...
  useEffect(() => { resolveFinancialMonth(user.id, thisMonth(), scope).then((r) => setMonth(r.month)); }, [user.id, scope]);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (table === 'income') {
        ...
        if (incomeSource === 'standard') await materialiseIncome(user.id, month, scope);
        const data = { user_id: user.id, month, name: row.name, amount: Number(row.amount), type: row.type || 'salary', scope };
    ...
    <div className="page page-wide">
      ...
      <div className="budget-visual-grid">
        ...
        <section className="card chart-card">
          ...
          {scope === 'household' ? (allocationData.length ? <div className="allocation-chart-wrap"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={allocationData} dataKey="value" nameKey="name" innerRadius="58%" outerRadius="82%" paddingAngle={3}>{allocationData.map((_,i)=><Cell key={i} fill={['#7657F5','#47C9AA','#65A8FF','#FF9F77','#D9A321','#A76BEF'][i%6]}/>)}</Pie><Tooltip formatter={(v)=>f(v)}/><Legend iconType="circle" wrapperStyle={{fontSize:12,textTransform:'capitalize'}}/></PieChart></ResponsiveContainer><div className="donut-center"><strong>{Math.round(totals.net ? totals.ta/totals.net*100 : 0)}%</strong><span>allocated</span></div></div> : <div className="empty-state"><div className="empty-title">Give your savings a job</div><p className="empty-body">Allocate savings to goals, investments or your emergency fund.</p></div>) : (catData.length ? <div className="allocation-chart-wrap"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={catData} dataKey="value" nameKey="name" innerRadius="58%" outerRadius="82%" paddingAngle={3}>{catData.map((_,i)=><Cell key={i} fill={['#7657F5','#47C9AA','#65A8FF','#FF9F77','#D9A321','#A76BEF'][i%6]}/>)}</Pie><Tooltip formatter={(v)=>f(v)}/><Legend iconType="circle" wrapperStyle={{fontSize:12,textTransform:'capitalize'}}/></PieChart></ResponsiveContainer><div className="donut-center"><strong>{f(totals.te)}</strong><span>operating costs</span></div></div> : <div className="empty-state"><div className="empty-title">Add normal business costs</div><p className="empty-body">Payroll, software, rent and other operating costs appear here.</p></div>)}
        </section>
      </div>
