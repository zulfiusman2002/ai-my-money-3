...
export default function Budget() {
  ...
  return (
    <div className="page page-wide">
      ...
      <PageIntro eyebrow="Where the month went — and where it should go" title="Budget" subtitle="A clear monthly view of income, spending and the money you deliberately put to work." action={<div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap:'wrap', justifyContent:'flex-end' }}><button className="chip" onClick={() => setMonth(shiftMonth(month, -1))}>←</button><span style={{ fontSize: '1rem', minWidth: 140, textAlign: 'center', fontWeight: 700 }}>{monthName(month)}</span><button className="chip" onClick={() => setMonth(shiftMonth(month, 1))} disabled={month >= thisMonth()}>→</button><button className="btn btn-primary btn-sm" onClick={() => setEditing({ table:'expense', row:{ category:'other', type:'variable', description:'', amount:'' } })}>+ Add expense</button></div>} />
      <MiloCoach mode="builder" eyebrow="Builder Milo · monthly foundations"
        title={totals.net >= 0 ? `You have ${f(totals.net)} left to put to work.` : `This month is ${f(Math.abs(totals.net))} over budget.`}
        ...
        facts={[
          { label:'Income', value:f(totals.ti), detail:`Recorded for ${monthName(month)}` },
          { label:'Built costs', value:f(totals.te), detail:`${totals.ti ? Math.round(totals.te/totals.ti*100) : 0}% of income` },
          { label:'Available', value:f(Math.max(0,totals.net)), detail:`${totals.rate.toFixed(0)}% savings rate` },
        ]}
