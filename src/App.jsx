...
const NAV_ITEMS = [
  ...
  { to: '/app/timeline',     label: 'Timeline',    icon: 'timeline', hint: 'Your connected history' },
...
const MORE_ITEMS = NAV_ITEMS.filter((x) => ['/app/goals', '/app/networth', '/app/timeline', '/app/projector'].includes(x.to));
    ...
    <div className="app-shell-v3">
      <aside className="desktop-sidebar">
        <button className="side-brand" onClick={() => nav('/app')} aria-label="MoneyMilo home">
          <MiloLogo size={76} />
          <span className="brand-word">Money<span>Milo</span><small className="brand-version">2.1</small></span>
        </button>
...
function Protected({ children }) {
  ...
  return <BrainProvider><Shell>{children}</Shell></BrainProvider>;
}
  ...
  return (
    <ErrorBoundary>
      ...
      <Routes>
        ...
        <Route path="/app/timeline" element={<Protected><Timeline /></Protected>} />
