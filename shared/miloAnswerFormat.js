...
function normaliseObject(value, disclaimer) {
  ...
  const visual = answer.visual && typeof answer.visual === 'object' && !Array.isArray(answer.visual) ? answer.visual : null;
  const visualProgress = visual?.progress_pct == null || visual.progress_pct === '' ? null : Math.max(0, Math.min(100, Number(visual.progress_pct)));
  ...
  return {
    ...
    visual: visual ? {
      kind: ['score','affordability','goal','comparison','cashflow','none'].includes(visual.kind) ? visual.kind : 'none',
      eyebrow: visual.eyebrow || '',
      headline: visual.headline || '',
      primary_label: visual.primary_label || '',
      primary_value: visual.primary_value || '',
      secondary_label: visual.secondary_label || '',
      secondary_value: visual.secondary_value || '',
      progress_pct: Number.isFinite(visualProgress) ? visualProgress : null,
      note: visual.note || '',
    } : null,
...
function salvage(text, disclaimer) {
  ...
  return normaliseObject({
    ...
    verdict,
    visual: objectField(cleaned, 'visual'),
...
export function answerToPlainText(answer) {
  ...
  if (value.visual?.headline) lines.push(`${value.visual.headline}${value.visual.primary_value ? ` — ${value.visual.primary_label}: ${value.visual.primary_value}` : ''}`);
