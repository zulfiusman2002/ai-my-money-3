...
async function resolveContextMonth(db, userId, requested = monthStr()) {
  const { data: current } = await db.from('expenses').select('month').eq('user_id', userId).eq('month', requested).limit(1);
  if (current?.length) return requested;
  const { data: latest } = await db.from('expenses').select('month').eq('user_id', userId).order('month', { ascending: false }).limit(1);
