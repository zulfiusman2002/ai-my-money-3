...
import { buildFinancialBrain, buildBrainContext } from '../../../shared/financialBrain.js';
...
console.log('[MONEYMILO] Function loaded.', { MODEL_FAST, MODEL_FULL });
...
export async function callClaude({ system, messages, maxTokens = 400, useFast = true }) {
  ...
  try {
    ...
    if (e.name === 'AbortError') throw httpError(504, "Milo's analysis timed out. Try a smaller review.");
    throw e;
  } finally {
    clearTimeout(timer);
  }
...
export const ADVISOR_SYSTEM = `You are Milo, the intelligent money companion inside "MoneyMilo".
Rules:
...
function calendarMonthsUntil(dateStr, now = new Date()) {
  if (!dateStr) return null;
  const target = new Date(`${dateStr}T23:59:59`);
  if (Number.isNaN(target.getTime())) return null;
  const raw = (target.getFullYear() - now.getFullYear()) * 12 + (target.getMonth() - now.getMonth());
  return Math.max(0, raw + (target.getDate() >= now.getDate() ? 1 : 0));
}

const LIQUID = new Set(['cash', 'gold']);
const SEMI   = new Set(['etf', 'uk_stocks', 'us_stocks', 'indian_stocks', 'mutual_funds', 'crypto', 'bonds']);

async function fetchIncome(db, userId, month, scope = 'household') {
  const [rec, std] = await Promise.all([
    db.from('income_records').select('amount, name, type, scope').eq('user_id', userId).eq('month', month).eq('scope', scope),
    db.from('income_sources').select('amount, name, type, scope').eq('user_id', userId).eq('is_active', true).eq('scope', scope),
  ]);
  return rec.data?.length ? rec.data : (std.data || []);
}

async function resolveContextMonth(db, userId, requested = monthStr(), scope = 'household') {
  const { data: current } = await db.from('expenses').select('month,scope').eq('user_id', userId).eq('month', requested).eq('scope', scope).limit(1);
  if (current?.length) return requested;
  const { data: latest } = await db.from('expenses').select('month,scope').eq('user_id', userId).eq('scope', scope).order('month', { ascending: false }).limit(1);
  return latest?.[0]?.month || requested;
}

// ── Lightweight per-analysis context builders ─────────────────────────────────

async function ctxBudget(db, userId) {
  const month = await resolveContextMonth(db, userId);
...
async function ctxGoals(db, userId) {
  ...
  const gLines = (goals.data || []).map((g) => {
    ...
    const ma = calendarMonthsUntil(g.target_date);
...
export async function buildContext(db, userId) {
  ...
  const topExpenses = I.budget.topExpenses.map((e) => `${e.description || e.category || 'Expense'} ${I.f(e.amount)}`).join(', ') || 'none';
  const portfolio = I.portfolio.items.map((item) => `${item.type} ${I.f(item.value)}${item.stale ? ' STALE' : ''}`).join(', ') || 'none';
  const assets = I.assets.rows.map((asset) => `${asset.name}(${asset.asset_class}) ${I.f(asset.converted_value)}`).join(', ') || 'none';
  const liabilities = I.liabilities.rows.map((row) => `${row.name} ${I.f(row.amount)}`).join(', ') || 'none';
  const goals = I.goals.map((goal) => `${goal.name}: ${I.f(goal.current)}/${I.f(goal.target)} at ${I.f(goal.monthly)}/mo, ${goal.onTrack === true ? 'on track' : goal.onTrack === false ? 'behind' : 'no deadline check'}`).join('; ') || 'none';
  const brain = buildFinancialBrain(I);
  return `FULL CONNECTED REVIEW ${I.month} (${I.base}, ${I.profile.profile_scope || I.profile.tracker_type || 'individual'}, scope ${I.scope}):
Income: ${I.f(I.budget.totalIncome)} | Expenses: ${I.f(I.budget.totalExpenses)} | Surplus: ${I.f(I.budget.netSavings)} | Savings rate: ${I.budget.savingsRate == null ? 'unknown' : `${I.budget.savingsRate.toFixed(1)}%`}
Top expenses: ${topExpenses}
Savings allocated: ${I.f(I.budget.savingsAllocated)}
Investments: ${portfolio}
Other assets: ${assets}
Liabilities: ${liabilities}
Net worth: ${I.f(I.netWorth)}${I.mom ? ` | Change vs previous snapshot: ${I.f(I.mom.netWorth)}` : ''}
Goals: ${goals}
Learning: ${I.learning.lessonsDone} lessons, ${I.learning.streak.current_streak || 0}-day streak, ${I.learning.streak.total_xp || 0} XP.
${buildBrainContext(brain, I.f)}`;
}

export async function buildAnalysisContext(db, userId, type) {
  const connected = await buildContext(db, userId);
  return `ANALYSIS FOCUS: ${type}. Use the connected model below and explain cross-module consequences where material.
...
export async function computeIntelligence(db, userId) {
  const profile = await db.from('user_profiles').select('*').eq('user_id', userId).maybeSingle();
  const p = profile.data || {};
  const scope = p.profile_scope === 'business' ? 'business' : 'household';
  const month = await resolveContextMonth(db, userId, monthStr(), scope);
  const [incomeRec, incomeStd, expenses, savingsAlloc, goals, liabilities, snaps, assets, progress, streak, prevSnap, modules] =
    await Promise.all([
      db.from('income_records').select('*').eq('user_id', userId).eq('month', month).eq('scope', scope),
      db.from('income_sources').select('*').eq('user_id', userId).eq('is_active', true).eq('scope', scope),
      db.from('expenses').select('*').eq('user_id', userId).eq('month', month).eq('scope', scope),
      scope === 'household' ? db.from('savings_allocations').select('*').eq('user_id', userId).eq('month', month) : Promise.resolve({ data: [], error: null }),
      db.from('goals').select('*').eq('user_id', userId).eq('status', 'active'),
      db.from('liabilities').select('*').eq('user_id', userId).eq('scope', scope),
  ...
  const goalStates = (goals.data || []).map((g) => {
    ...
    const monthsAvailable = calendarMonthsUntil(g.target_date, new Date(today));
  ...
  return {
    month, scope, base, sym, f, profile: p,
