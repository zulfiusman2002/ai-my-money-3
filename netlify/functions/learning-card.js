...
export const handler = wrap(async (event) => {
  ...
  // Deterministic pick: MoneyMilo Brain recommendation first, then the trigger list, then sequential.
  let pick = remaining.find((lesson) => lesson.module_id === brain.lesson.moduleId) || null, trigger = I.triggers.find((item) => item.module === brain.lesson.moduleId) || null;
  ...
  let reason = brain.lesson.reason || (trigger ? `Picked for you because ${trigger.why}.` : 'Next up on your learning path.');
  try {
  ...
  return json(200, { lesson_id: pick.id, reason, trigger: trigger?.code || brain.lesson.code || null, brain_score: brain.score, brain_priority: brain.priorities[0] || null });
});
